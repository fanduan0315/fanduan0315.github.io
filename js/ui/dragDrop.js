 ...
 *
 * 【元素创建】
 * - createDraggablePlayer(player): 创建可拖拽的玩家元素（用于未分配玩家列表）
 *
 * 【拖放区域设置】
 * - setupRoomDropZone(roomId, maxPlayers, countElementId, module, roomProp): 设置房间拖放区域，支持module1-4，拖放后自动从玩家列表中移除
 *
 * 【玩家移动】
...
function createDraggablePlayer(player) {
  ...
  playerEl.className =
    "player-item bg-white border border-gray-300 rounded-full px-3 py-1 text-sm flex items-center justify-center cursor-move scale-hover";
